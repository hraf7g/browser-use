    ...
    if latest_successful_run is not None:
        ...
        items.append(
            TenderDetailsTimelineItem(
                ...
                summary=(
                    f"{source.name if source is not None else 'Source'} completed a successful crawl run."
                ),
            )
        )
    ...
    if match is not None:
        items.append(
            TenderDetailsTimelineItem(
                ...
                title='Match created',
                summary=_build_keywords_summary(match.matched_keywords or []),
            )
        )
...
def _build_keywords_summary(keywords: list[str]) -> str:
    """Return a compact, safe summary for matched keywords."""
    if not keywords:
        return 'No matched keywords were recorded for this tender.'

    joined = ', '.join(keywords)
    return f"Matched keywords: {joined}."
