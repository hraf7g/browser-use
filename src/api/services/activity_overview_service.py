...
def _build_summary(*, sources: list[Source]) -> ActivityOverviewSummary:
    ...
    for source in sources:
        status = (source.status or '').strip().lower()
        if status == 'healthy':
            healthy += 1
        elif status == 'failed':
            failed += 1
        ...
        if source.last_successful_run_at is not None:
            if (
                latest_successful_check_at is None
                or source.last_successful_run_at > latest_successful_check_at
            ):
    ...
    for match, tender, source in match_rows:
        ...
        items.append(
            ActivityFeedItem(
                ...
                title='Match created',
                summary=_build_match_summary(tender_title=tender.title, keywords=keywords),
                source_id=source.id,
                source_name=source.name,
                tender_id=tender.id,
...
def _build_match_summary(*, tender_title: str, keywords: list[str]) -> str:
    """Build a compact match summary for the activity feed."""
    if keywords:
        return f"{tender_title} matched {', '.join(keywords)}."
