...
def run() -> None:
	...
	with session_factory() as session:
		tender = session.execute(select(Tender).where(Tender.tender_ref == MANUAL_TENDER_REF)).scalar_one_or_none()

		if tender is None:
			raise ValueError(f"tender '{MANUAL_TENDER_REF}' was not found; run manual ingestion first")
		source = session.get(Source, tender.source_id)
		if source is None:
			raise ValueError(f"source '{tender.source_id}' was not found for tender '{tender.id}'")
