...
def run() -> None:
	...
	with session_factory() as session:
		...
		if first_target_user_created != 1:
			raise ValueError(f'expected exactly one target user match on first run, got {first_target_user_created}')

		if second_target_user_created != 0:
			raise ValueError(f'expected zero target user matches on second run, got {second_target_user_created}')
