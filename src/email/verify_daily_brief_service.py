...
def run() -> None:
    ...
    with session_factory() as session:
        ...
        if first_target_user_created != 1:
            raise ValueError(
                f"expected exactly one new target-user match on first run, got {first_target_user_created}"
            )

        if second_target_user_created != 0:
            raise ValueError(
                f"expected zero new target-user matches on second run, got {second_target_user_created}"
            )
        ...
        if len(brief.match_ids) != 1:
            raise ValueError(
                f"expected exactly one match_id in brief, got {len(brief.match_ids)}"
            )

        if len(brief.tender_ids) != 1:
            raise ValueError(
                f"expected exactly one tender_id in brief, got {len(brief.tender_ids)}"
            )
