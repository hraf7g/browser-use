...
def run() -> None:
	...
	with session_factory() as session:
		...
		if first_target_user_created != 1:
			raise ValueError(f'expected exactly one new target-user match, got {first_target_user_created}')
		...
		if email_delivery.delivery_type != 'daily_brief':
			raise ValueError(f"expected delivery_type 'daily_brief', got {email_delivery.delivery_type}")

		if email_delivery.status != 'sent':
			raise ValueError(f"expected delivery status 'sent', got {email_delivery.status}")

		if email_delivery.match_count != 1:
			raise ValueError(f'expected EmailDelivery.match_count to be 1, got {email_delivery.match_count}')
