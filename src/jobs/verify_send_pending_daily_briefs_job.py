...
def run() -> None:
	...
	if matches_created < 1:
		raise ValueError(f'expected at least one TenderMatch row before daily brief job, got {matches_created}')
	...
	with session_factory() as session:
		...
		if latest_delivery.status != 'sent':
			raise ValueError(f"expected latest daily brief delivery status 'sent', got {latest_delivery.status}")
	...
	print(f'verify_daily_brief_job_processed_user_count={result.processed_user_count}')
	...
	print(f'verify_daily_brief_job_target_match_sent_at={target_match.sent_at.isoformat()}')
