...
def run() -> None:
    ...
    if matches_created < 1:
        raise ValueError(
            f"expected at least one TenderMatch row before instant-alert idempotency test, got {matches_created}"
        )
    ...
    with session_factory() as session:
        user_deliveries = session.execute(
    ...
    if len(user_deliveries) != 1:
        raise ValueError(
            f"expected exactly one instant-alert delivery for target user, got {len(user_deliveries)}"
        )
    ...
    print(
        "verify_instant_alert_job_idempotency_user_delivery_count="
        f"{len(user_deliveries)}"
    )
    print(
        "verify_instant_alert_job_idempotency_target_match_alerted_at="
        f"{target_match.alerted_at.isoformat()}"
    )
    print(
        f"verify_instant_alert_job_idempotency_target_match_sent_at={target_match.sent_at}"
    )
