...
def run() -> None:
	...
	with session_factory() as session:
		...
		persisted_tender = session.execute(select(Tender).where(Tender.id == tender.id)).scalar_one()

	if target_match is None:
		raise ValueError('expected target TenderMatch row was not created')

	if first_created < 1:
		raise ValueError(f'expected at least one TenderMatch row on first run, got {first_created}')

	if second_created != 0:
		raise ValueError(f'expected zero new TenderMatch rows on second run, got {second_created}')

	if persisted_match_count != 1:
		raise ValueError(f'expected exactly one persisted user+tender TenderMatch row, got {persisted_match_count}')
	...
	print(f'verify_match_recent_tenders_job_matched_keywords={target_match.matched_keywords}')
