...
def run() -> None:
    ...
    if matches_created < 2:
        raise ValueError(
            f"expected at least two TenderMatch rows before instant-alert job, got {matches_created}"
        )
    ...
    print(
        f"verify_instant_alert_job_processed_user_count={result.processed_user_count}"
    )
    print(
        f"verify_instant_alert_job_sent_delivery_count={result.sent_delivery_count}"
    )
    print(
        f"verify_instant_alert_job_skipped_user_count={result.skipped_user_count}"
    )
    ...
    print(
        f"verify_instant_alert_job_eligible_delivery_count={len(eligible_deliveries)}"
    )
    print(
        f"verify_instant_alert_job_blocked_delivery_count={len(blocked_deliveries)}"
    )
    print(
        "verify_instant_alert_job_eligible_match_alerted_at="
        f"{eligible_match.alerted_at.isoformat()}"
    )
