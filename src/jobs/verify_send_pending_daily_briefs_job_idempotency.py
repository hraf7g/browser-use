...
def run() -> None:
    ...
    if matches_created < 1:
        raise ValueError(
            f"expected at least one TenderMatch row before daily brief idempotency test, got {matches_created}"
        )
    ...
    if len(user_deliveries) != 1:
        raise ValueError(
            f"expected exactly one daily brief delivery for target user, got {len(user_deliveries)}"
        )
    ...
    print(
        "verify_daily_brief_job_idempotency_user_delivery_count="
        f"{len(user_deliveries)}"
    )
    print(
        "verify_daily_brief_job_idempotency_target_match_sent_at="
