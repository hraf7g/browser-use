...
from src.email.email_delivery_outbox_service import (
    ...
    queue_daily_brief_delivery_for_user,
)
    ...
    with session_factory() as lock_session:
        ...
        try:
            ...
            with session_factory() as read_session:
                pending_user_ids = _list_pending_daily_brief_user_ids(read_session)
            ...
            for user_id in pending_user_ids:
                ...
                with session_factory() as queue_session:
                    ...
                    if (
                        ...
                        or not preference.daily_brief_enabled
                    ):
                        skipped_user_count += 1
                        queue_session.rollback()
                        continue

                    queued_result = queue_daily_brief_delivery_for_user(
                        queue_session,
                        user_id=user_id,
                    )
...
def _mark_delivery_failed(
    *,
    ...
    """Persist a failed daily-brief delivery finalization."""
...
def _list_pending_daily_brief_user_ids(session) -> list:
    """Return distinct user ids with unqueued unsent tender matches in stable order."""
    return list(
        session.execute(
