...
def run() -> None:
	"""
	Perform a production-grade persisted verification of the Saudi Etimad run service.
	...
	with session_factory() as session:
		...
		result = run_saudi_etimad_source(session=session)
		session.commit()
	...
	with session_factory() as session:
		...
		if result.status != 'success':
			raise ValueError(
				'expected successful Saudi Etimad source run, got '
				f'{result.status} (failure_step={result.failure_step}, '
				f'failure_reason={result.failure_reason})'
			)

		if crawl_run.status != 'success':
			raise ValueError(f'expected persisted crawl_run.status=success, got {crawl_run.status}')
		...
		if populated_search_text_count == 0:
			raise ValueError('expected persisted tenders with populated search_text, got zero')
		...
		sample_tender_with_search = next(
			(tender for tender in tenders_after if isinstance(tender.search_text, str) and tender.search_text.strip()),
			None,
		)
		...
		print(f'verify_run_source_before_last_successful_run_at={before_last_successful_run_at}')
		...
		print(f'verify_run_source_after_last_successful_run_at={source_after.last_successful_run_at.isoformat()}')
		...
		print(f'verify_run_sample_search_text={sample_tender_with_search.search_text[:120]}')
