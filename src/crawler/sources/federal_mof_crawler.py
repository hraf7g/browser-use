...
class FederalMOFCrawler:
	...
	async def _navigate(self, page: Page) -> None:
		...
		try:
			...
			raise FederalMOFNavigationError(f"failed to navigate to '{self._config.listing_url}'") from exc

		if response is None:
			raise FederalMOFNavigationError('navigation returned no HTTP response')

		if response.status >= 400:
			raise FederalMOFNavigationError(f'navigation failed with HTTP status {response.status}')
	...
	async def _verify_page(self, page: Page) -> None:
		...
		if not matched_markers:
			raise FederalMOFNavigationError('page verification failed; expected text markers were not found')
