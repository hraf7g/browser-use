...
def run() -> None:
    """
    Perform a production-grade persisted verification of the Federal MOF
    run service.
    ...
    with session_factory() as session:
        ...
        if result.status != "success":
            raise ValueError(
                "expected successful Federal MOF source run, got "
        ...
        if result.accepted_row_count != (
            ...
            raise ValueError(
                "accepted_row_count does not equal created_tender_count + "
                "updated_tender_count"
            )
