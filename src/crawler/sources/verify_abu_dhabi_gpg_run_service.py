...
def run() -> None:
    """
    Perform a production-grade persisted verification of the Abu Dhabi GPG run service.

    This script verifies:
    - the enriched source run completes successfully
    - a CrawlRun row is persisted
    - source health fields are updated
    - accepted enriched rows are ingested into tenders
    - persisted tenders have populated search_text
    - persisted counts are internally consistent
    """
    ...
    with session_factory() as session:
        ...
        result = run_abu_dhabi_gpg_source(session=session)
        session.commit()
    ...
    with session_factory() as session:
        ...
        tenders_after = session.execute(
            select(Tender).where(Tender.source_id == source_after.id)
        ).scalars().all()
        ...
        if result.status != "success":
            raise ValueError(
                "expected successful Abu Dhabi GPG source run, got "
                f"{result.status} (failure_step={result.failure_step}, "
        ...
        print(
            "verify_run_source_before_last_successful_run_at="
            f"{before_last_successful_run_at}"
        )
        print(f"verify_run_before_tender_count={before_tender_count}")
        print(f"verify_run_result_source_name={result.source_name}")
        print(f"verify_run_result_status={result.status}")
        print(f"verify_run_result_run_identifier={result.run_identifier}")
        print(
            "verify_run_result_widget_crawled_row_count="
            f"{result.widget_crawled_row_count}"
        )
        print(
            "verify_run_result_detail_sampled_row_count="
            f"{result.detail_sampled_row_count}"
        )
        print(
            "verify_run_result_enriched_normalized_row_count="
            f"{result.enriched_normalized_row_count}"
        )
        print(f"verify_run_result_accepted_row_count={result.accepted_row_count}")
        print(
            "verify_run_result_review_required_row_count="
            f"{result.review_required_row_count}"
        )
        ...
        print(f"verify_run_result_failure_step={result.failure_step}")
        print(f"verify_run_result_failure_reason={result.failure_reason}")
        ...
        print(
            "verify_run_source_after_last_successful_run_at="
            f"{source_after.last_successful_run_at.isoformat()}"
        )
        ...
        print(
            "verify_run_sample_search_text="
            f"{sample_tender_with_search.search_text[:120]}"
        )
