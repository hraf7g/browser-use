...
from src.crawler.sources.saudi_etimad_detail_crawler import (
	SaudiEtimadDetailCrawlerError,
	run_saudi_etimad_detail_crawl,
)
from src.crawler.sources.saudi_etimad_enriched_normalizer import (
	SaudiEtimadEnrichedNormalizationError,
	normalize_saudi_etimad_enriched_items,
)
from src.crawler.sources.saudi_etimad_enriched_quality import (
	assess_saudi_etimad_enriched_payloads,
)
from src.db.models.crawl_run import CrawlRun
from src.db.models.source import Source
from src.ingestion.tender_ingestion_service import ingest_tender

SOURCE_NAME: Final[str] = 'Saudi Etimad'
MAX_FAILURE_REASON_LENGTH: Final[int] = 2000


class SaudiEtimadSourceRunError(ValueError):
	"""Raised when the Saudi Etimad source cannot be initialized."""


@dataclass(frozen=True)
class SaudiEtimadSourceRunResult:
	"""Structured result for one full Saudi Etimad source run."""
...
def run_saudi_etimad_source(session: Session) -> SaudiEtimadSourceRunResult:
	"""
	Run the Saudi Etimad source end-to-end.
	...
	run_identifier = f'saudi-etimad-{uuid4().hex}'
	...
	try:
		crawl_result = asyncio.run(run_saudi_etimad_crawl())
		...
		normalized_rows = normalize_saudi_etimad_enriched_items(
			source_id=source.id,
			items=detail_result.items,
		)
		normalized_row_count = len(normalized_rows)

		assessments = assess_saudi_etimad_enriched_payloads(normalized_rows)
		...
		return SaudiEtimadSourceRunResult(
...
def _get_source(session: Session) -> Source:
	"""Load the Saudi Etimad source from the database."""
	source = session.execute(select(Source).where(Source.name == SOURCE_NAME)).scalar_one_or_none()
	if source is None:
		raise SaudiEtimadSourceRunError(f"source '{SOURCE_NAME}' was not found; seed sources first")
	return source
...
def _classify_failure_step(exc: Exception) -> str:
	...
	if isinstance(exc, SaudiEtimadCrawlerError):
		return 'crawl'
	if isinstance(exc, SaudiEtimadDetailCrawlerError):
		return 'detail_enrichment'
	if isinstance(exc, SaudiEtimadEnrichedNormalizationError):
		return 'normalize'
