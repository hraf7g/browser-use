...
async def _run() -> None:
    """
    Perform an isolated production-grade verification of the Oman Tender Board ingestion pipeline.
    ...
    with session_factory() as session:
        ...
        if not accepted_assessments:
            raise ValueError(
                "expected at least one accepted Oman Tender Board row, got zero"
            )
...
def run() -> None:
    ...
    try:
        ...
        print(
            f"OMAN_TENDER_BOARD INGESTION PIPELINE VERIFICATION FAILED: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
