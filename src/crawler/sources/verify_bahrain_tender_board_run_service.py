...
def run() -> None:
	"""
	Perform a production-grade persisted verification of the Bahrain run service.

	This script verifies:
	    - the source run completes without raising
	    - a CrawlRun row is persisted
	    - source health fields are updated
	    - accepted rows, when present, are ingested into tenders
	    - ingested tenders have persisted search_text
	    - persisted counts remain internally consistent
	"""
	...
	with session_factory() as session:
		...
		result = run_bahrain_tender_board_source(session=session)
		session.commit()
