...
from src.crawler.sources.qatar_monaqasat_normalizer import (
	QatarMonaqasatNormalizationError,
	normalize_qatar_monaqasat_items,
)
from src.crawler.sources.qatar_monaqasat_quality import (
	assess_qatar_monaqasat_payloads,
)
from src.db.models.crawl_run import CrawlRun
from src.db.models.source import Source
from src.ingestion.tender_ingestion_service import ingest_tender

SOURCE_NAME: Final[str] = 'Qatar Monaqasat'
MAX_FAILURE_REASON_LENGTH: Final[int] = 2000


class QatarMonaqasatSourceRunError(ValueError):
	"""Raised when the Qatar Monaqasat source cannot be initialized."""


@dataclass(frozen=True)
class QatarMonaqasatSourceRunResult:
	"""Structured result for one full Qatar Monaqasat source run."""
...
def run_qatar_monaqasat_source(session: Session) -> QatarMonaqasatSourceRunResult:
	"""
	Run the Qatar Monaqasat source end-to-end.
	"""
	source = _get_source(session=session)
	started_at = datetime.now(UTC)
	run_identifier = f'qatar-monaqasat-{uuid4().hex}'
	...
	try:
		crawl_result = asyncio.run(run_qatar_monaqasat_crawl())
		...
		normalized_rows = normalize_qatar_monaqasat_items(
			source_id=source.id,
			items=detail_result.items,
		)
		normalized_row_count = len(normalized_rows)

		assessments = assess_qatar_monaqasat_payloads(normalized_rows)
		...
		return QatarMonaqasatSourceRunResult(
...
def _get_source(session: Session) -> Source:
	"""Load the Qatar Monaqasat source from the database."""
	source = session.execute(select(Source).where(Source.name == SOURCE_NAME)).scalar_one_or_none()
	if source is None:
		raise QatarMonaqasatSourceRunError(f"source '{SOURCE_NAME}' was not found; seed sources first")
	return source
...
def _classify_failure_step(exc: Exception) -> str:
	...
	if isinstance(exc, QatarMonaqasatCrawlerError):
		return 'crawl'
	if isinstance(exc, QatarMonaqasatDetailCrawlerError):
		return 'detail_enrichment'
	if isinstance(exc, QatarMonaqasatNormalizationError):
		return 'normalize'
