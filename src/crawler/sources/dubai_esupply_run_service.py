...
from src.crawler.sources.dubai_esupply_normalizer import (
	DubaiESupplyNormalizationError,
...
SOURCE_NAME: Final[str] = 'Dubai eSupply'
MAX_FAILURE_REASON_LENGTH: Final[int] = 2000


class DubaiESupplySourceRunError(ValueError):
	"""Raised when the Dubai eSupply source itself cannot be initialized."""


@dataclass(frozen=True)
class DubaiESupplySourceRunResult:
	"""Structured result for one full Dubai eSupply source run."""
...
def run_dubai_esupply_source(session: Session) -> DubaiESupplySourceRunResult:
	"""
	Run the Dubai eSupply source end-to-end.
	...
	run_identifier = f'dubai-esupply-{uuid4().hex}'
	...
	try:
		crawl_result = asyncio.run(run_dubai_esupply_crawl())
		crawled_row_count = crawl_result.total_rows
		...
		return DubaiESupplySourceRunResult(
			source_id=source.id,
		...
		# Update source health for degradation
...
def _get_source(session: Session) -> Source:
	"""Load the Dubai eSupply source from the database."""
	source = session.query(Source).filter(Source.name == SOURCE_NAME).one_or_none()
	if source is None:
		raise DubaiESupplySourceRunError(f"source '{SOURCE_NAME}' was not found; seed sources first")
	return source


def _classify_failure_step(exc: Exception) -> str:
	"""Classify failure stage for crawl-run diagnostics."""
	if isinstance(exc, DubaiESupplyCrawlerError):
		return 'crawl'
	if isinstance(exc, DubaiESupplyNormalizationError):
		return 'normalize'
