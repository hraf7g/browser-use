...
class SaudiMisaQualityAssessment:
    """
    Deterministic quality assessment for one normalized Saudi MISA payload.
...
def assess_saudi_misa_payload(
    payload: TenderIngestionInput,
) -> SaudiMisaQualityAssessment:
    """
    Assess one normalized Saudi MISA payload for downstream data quality risk.
    ...
    Saudi MISA notes:
        - the stronger competitions table now provides real tender references and,
          on most rows, real publish/closing dates
        - `MISA` is a source-level visible issuer on the official procurements page
        - rows using the unknown closing sentinel still must not pass silently
        - missing category and missing opening_date reduce confidence but do not
    ...
    return SaudiMisaQualityAssessment(
        payload=payload,
...
def assess_saudi_misa_payloads(
    payloads: Sequence[TenderIngestionInput],
) -> tuple[SaudiMisaQualityAssessment, ...]:
    """Assess a sequence of normalized Saudi MISA payloads."""
    return tuple(assess_saudi_misa_payload(payload) for payload in payloads)
