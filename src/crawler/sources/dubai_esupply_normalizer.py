...
SIMPLE_SLASH_PATTERN = re.compile(r'^(?P<ref>[^/]{2,100})\s*/\s*(?P<title>.+)$')
NUMERIC_HYPHEN_PATTERN = re.compile(r'^(?P<ref>\d{4,})\s*-\s*(?P<title>.+)$')
NUMERIC_SPACE_PATTERN = re.compile(r'^(?P<ref>\d{4,})\s+(?P<title>.+)$')
...
def normalize_dubai_esupply_row(
	*,
	...
	- the visible row action href is preferred as source_url when present
	- the listing URL is used only as a fallback
	"""
	...
	canonical_source_url = _canonicalize_dubai_source_url(_normalize_source_url(row))
...
def _normalize_source_url(row: DubaiESupplyListingRow) -> str:
	"""Prefer the row action href and fall back to the listing page URL."""
	action_href = _none_if_empty(row.action_href)
	...
	if slash_match is not None:
		...
		if tender_ref and title and _is_high_confidence_tender_ref(tender_ref):
			...
			if nested_hyphen_match is not None:
				...
				if nested_ref and nested_title and _is_high_confidence_tender_ref(nested_ref):
...
def _parse_dubai_closing_date(raw_value: str) -> datetime:
	...
	try:
		local_dt = datetime.strptime(cleaned, '%d/%m/%Y %H:%M').replace(tzinfo=DUBAI_TIMEZONE)
	except ValueError as exc:
		raise DubaiESupplyDateParseError(f"could not parse closing date '{cleaned}'") from exc
