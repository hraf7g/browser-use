...
def run() -> None:
	"""
	Perform a production-grade persisted verification of the Saudi MISA run service.
	...
	with session_factory() as session:
		...
		result = run_saudi_misa_source(session=session)
		session.commit()
	...
	with session_factory() as session:
		...
		if result.status != 'success':
			raise ValueError(
				'expected successful Saudi MISA source run, got '
