...
from src.crawler.sources.bahrain_tender_board_enriched_normalizer import (
	BahrainTenderBoardEnrichedNormalizationError,
	normalize_bahrain_tender_board_enriched_items,
)
from src.crawler.sources.bahrain_tender_board_enriched_quality import (
	assess_bahrain_tender_board_enriched_payloads,
)
from src.db.models.crawl_run import CrawlRun
from src.db.models.source import Source
from src.ingestion.tender_ingestion_service import ingest_tender

SOURCE_NAME: Final[str] = 'Bahrain Tender Board'
MAX_FAILURE_REASON_LENGTH: Final[int] = 2000


class BahrainTenderBoardSourceRunError(ValueError):
	"""Raised when the Bahrain Tender Board source cannot be initialized."""


@dataclass(frozen=True)
class BahrainTenderBoardSourceRunResult:
	"""Structured result for one full Bahrain Tender Board enriched source run."""
...
def run_bahrain_tender_board_source(
	session: Session,
) -> BahrainTenderBoardSourceRunResult:
	"""
	Run the Bahrain Tender Board source end-to-end.

	Flow:
	    - live dashboard crawl
	    - live detail-page enrichment across the crawled items
	...
	run_identifier = f'bahrain-tender-board-{uuid4().hex}'
	...
	try:
		crawl_result = asyncio.run(run_bahrain_tender_board_crawl())
		...
		normalized_rows = normalize_bahrain_tender_board_enriched_items(
			source_id=source.id,
			items=detail_result.items,
		)
		normalized_row_count = len(normalized_rows)

		assessments = assess_bahrain_tender_board_enriched_payloads(normalized_rows)
		...
		return BahrainTenderBoardSourceRunResult(
...
def _get_source(session: Session) -> Source:
	"""Load the Bahrain Tender Board source from the database."""
	source = session.execute(select(Source).where(Source.name == SOURCE_NAME)).scalar_one_or_none()
	if source is None:
		raise BahrainTenderBoardSourceRunError(f"source '{SOURCE_NAME}' was not found; seed sources first")
	return source
...
def _classify_failure_step(exc: Exception) -> str:
	...
	if isinstance(exc, BahrainTenderBoardCrawlerError):
		return 'crawl'
	if isinstance(exc, BahrainTenderBoardDetailCrawlerError):
		return 'detail_enrichment'
	if isinstance(exc, BahrainTenderBoardEnrichedNormalizationError):
		return 'normalize'
