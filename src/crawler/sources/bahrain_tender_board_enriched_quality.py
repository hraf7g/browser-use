...
class BahrainTenderBoardEnrichedQualityAssessment:
	"""
	Deterministic quality assessment for one enriched Bahrain Tender Board payload.
	"""
...
def assess_bahrain_tender_board_enriched_payload(
	payload: TenderIngestionInput,
) -> BahrainTenderBoardEnrichedQualityAssessment:
	"""
	Assess one enriched Bahrain payload for downstream data quality risk.

	Bahrain enriched notes:
	    - missing published_at reduces confidence but does not force review
	    - missing closing_date still forces review because the dashboard only exposes
	      a relative time-left value, not a safe absolute deadline
	"""
	flags: list[str] = []
	score = 100
	...
	if payload.issuing_entity == BAHRAIN_TENDER_BOARD_CONFIG.source_name:
	...
	return BahrainTenderBoardEnrichedQualityAssessment(
		payload=payload,
...
def assess_bahrain_tender_board_enriched_payloads(
	payloads: Sequence[TenderIngestionInput],
) -> tuple[BahrainTenderBoardEnrichedQualityAssessment, ...]:
	"""Assess a sequence of enriched Bahrain payloads."""
	return tuple(assess_bahrain_tender_board_enriched_payload(payload) for payload in payloads)
